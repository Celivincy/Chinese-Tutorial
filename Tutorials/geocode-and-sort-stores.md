...
现在你的地理编码器正在正确运行，你可以写代码来为你搜索到的结果添加一个定位点地图中。所有的代码会添加在 `map.on('load', function (e) { ... });` 里你上一步代码的后面。步骤如下：首先，你需要使用 `map.addSource()` 方法来添加一个空的源到你存储地理编码器结果的地方，接下来你需要通过`map.addLayer()`方法来使用这个源中的样式图层。
